...
function setBuffers(obj, program) {
  ...
  // WebGL gets mad bc vColor is unused
...
Cube.init = function (program) {
  ...
  for (var i = 0; i < vertices1.length; i++) {
    this.pointsArray.push(
      vec4(vertices1[count], vertices1[count + 1], vertices1[count + 2], 1.0)
    );
    this.normalsArray.push(
      vec3(normals1[count], normals1[count + 1], normals1[count + 2])
    );
    this.colorsArray.push(
      vec4(colors1[count], colors1[count + 1], colors1[count + 2], 1.0)
    );

    this.texCoordsArray.push(
      cubeTexCoord[texCount],
      cubeTexCoord[texCount + 1]
    );
